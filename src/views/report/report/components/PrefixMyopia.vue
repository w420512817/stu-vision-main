<template>
  <div class="w-full">
    ...
    <div v-if="areaLevel == 2 || areaLevel == 1">
      <LineChartReport height="250px" width="95%" :extend="myExtend2" :label-map="labelMap2" :chart-data="chartData2" />
      <p class="text-center m-t-5 m-b-10">{{ areaName }}各县（市、区）分学段近视率</p>
      <p class="text-center m-b-3 font-bold">{{ areaName }}各县（市、区）分学段近视情况</p>
      ...
      <p class="text-center m-t-5 m-b-10">{{ areaName }}各县（市、区）分学段高度近视率</p>
      <p class="text-center m-b-3 font-bold">{{ areaName }}各县（市、区）分学段高度近视情况</p>
      <n-table :single-line="false" class="text-center">
        <thead>
          <tr>
            <th>序号</th>
    ...
    <div v-if="areaLevel == 3">
      ...
      <p class="text-center m-t-5 m-b-10">{{ areaName }}各学段近视人数</p>
      <p class="text-center m-b-3 font-bold">{{ areaName }}各学段视力情况-男生</p>
      <n-table :single-line="false" class="text-center">
        <thead>
          <tr>
            <th>序号</th>
      ...
      <p class="text-center m-b-3 m-t-10 font-bold">{{ areaName }}各学段视力情况-女生</p>
      <n-table :single-line="false" class="text-center">
        <thead>
          <tr>
            <th>序号</th>
      ...
      <p class="text-center m-b-3 m-t-10 font-bold">{{ areaName }}各学段视力情况-总</p>
      <n-table :single-line="false" class="text-center">
        <thead>
          <tr>
            <th>序号</th>
      ...
      <p class="text-center m-b-10">{{ areaName }}各学段近视程度分布情况</p>
      <p class="text-center m-b-3 m-t-10 font-bold">{{ areaName }}各学段近视程度分布情况</p>
      <n-table :single-line="false" class="text-center">
        <thead>
          <tr>
            <th>序号</th>
...
import { line_per, bar_num } from '@common/settings/chartSetting';
import { getMyopiaInfoByPrefx, getMyopiaInfoByPrefx_r } from '@/api/report/index';
import { getMyopiaInfoByPrefxPro } from '@/api/report/index_province';
...
if (areaLevel == 1) {
  getMyopiaInfoByPrefxPro({
    ...
    d.messageList.forEach((v, i) => {
      ...
      let dec = `${i == 0 ? '' : '；'}${v.prefxName}近视人数${v.myopiaStdCount}人，近视率为${floatToPercent(
        v.myopiaRate
      )}，${v.maxRegionName}最高为${floatToPercent(v.maxRegionMyopiaRate)}，${v.minRegionName}最低为${floatToPercent(
        v.minRegionMyopiaRate
      )}`;
      str += dec;
      let dec2 = `${i == 0 ? '' : '；'}${v.prefxName}高度近视人数${
        v.highMyopiaStdCount
      }人，高度近视率为${floatToPercent(v.highMyopiaRate)}，${v.highMaxRegionName}最高为${floatToPercent(
        v.highMaxRegionMyopiaRate
      )}，${v.highMinRegionName}最低为${floatToPercent(v.highMinRegionMyopiaRate)}`;
      str2 += dec2;
    });
  ...
  getMyopiaInfoByPrefx_r({
    ...
    d.manList.forEach((v, i) => {
      state.wmCounts.push({
        ...
        count: v.myopiaStdCount * 1 + d.womanList[i].myopiaStdCount * 1,
        prefixName: v.prefixName
      });
    });
    chartData3.rows = state.wmCounts.filter(v => v.prefixName != '合计');
    chartData4.rows = d.allList.filter(v => v.prefixName != '合计');
  });
}
</script>
