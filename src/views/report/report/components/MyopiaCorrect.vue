<template>
  <div ref="MyopiaCorrect" class="w-full">
    ...
    <p class="text-center m-b-10">
      {{ areaName }}各{{ areaLevel == 2 || areaLevel == 1 ? '地区' : areaLevel == 3 ? '学段' : '年级' }}近视矫正情况
    </p>
    ...
    <n-table :single-line="false" class="text-center">
      <thead>
        <tr>
          <th>序号</th>
          <th>
            {{ areaLevel == 2 || areaLevel == 1 ? '县(市、区)' : areaLevel == 3 ? '学段' : '年级' }}
          </th>
          <th>
            足矫人数
            <div v-if="areaLevel == 4">(男/女)</div>
          </th>
          <th>
            足矫率
            <div v-if="areaLevel == 4">(男/女)</div>
      ...
      <tbody v-if="areaLevel == 2 || areaLevel == 1">
        <tr v-for="(item, index) in state.detail.regionMyopiaCorrectInfos" :key="item.region">
          <td>{{ index + 1 }}</td>
          <td>{{ item.region }}</td>
          <td>{{ item.correctMyopiaStdCount }}</td>
          <td>{{ floatToPercent(item.correctMyopiaRate) }}</td>
          <td>{{ item.udMyopiaStdCount }}</td>
          <td>{{ floatToPercent(item.udMyopiaRate) }}</td>
          <td>{{ item.unMyopiaStdCount }}</td>
          <td>{{ floatToPercent(item.unMyopiaRate) }}</td>
        </tr>
      </tbody>
      <tbody v-if="areaLevel == 3">
        <tr v-for="(item, index) in state.detail.schoolMyopiaCorrectInfos" :key="item.prefixName">
          <td>{{ index + 1 }}</td>
    ...
    <div v-if="areaLevel == 2 || areaLevel == 3">
      <div class="m-b-15"></div>
      <BarChartReport :extend="myExtend2" height="250px" />
      <p class="text-center m-b-10">{{ areaName }}各年级近视矫正情况</p>
      <p class="text-center m-b-3 font-bold">{{ areaName }}各年级近视矫正情况</p>
      <n-table :single-line="false" class="text-center">
        <thead>
          <tr>
            <th>序号</th>
...
import { getMyopiaCorrectInfo, getMyopiaCorrectInfo_r, getMyopiaCorrectInfo_s } from '@/api/report/index';
import { getMyopiaCorrectInfoPro } from '@/api/report/index_province';
import { useReportStore } from '@/store/modules/report';
import { floatToPercent } from '../data';

const userStore = useReportStore();
let { planId, areaId, areaLevel, areaName, schoolId, schoolType } = userStore.getAreaData;
let state = reactive({
  decStr: '',
  detail: {}
});
...
if (areaLevel == 1) {
  getMyopiaCorrectInfoPro({
    ...
    let arr2 = [];
    let arr3 = [];
    d.regionMyopiaCorrectInfos.forEach(v => {
      arr.push(v.region);
      arr1.push(v.correctMyopiaRate);
      arr2.push(v.udMyopiaRate);
      arr3.push(v.unMyopiaRate);
    });
    ...
    d.gradeMyopiaCorrectInfos.forEach(v => {
      ...
      arr2_1.push(v.udMyopiaRate);
      arr3_1.push(v.unMyopiaRate);
    });
  ...
  getMyopiaCorrectInfo_r({
    ...
    d?.schoolMyopiaCorrectInfos?.forEach(v => {
      arr.push(v.prefixName);
      arr1.push(v.correctMyopiaRate);
      arr2.push(v.udMyopiaRate);
      arr3.push(v.unMyopiaRate);
    });
