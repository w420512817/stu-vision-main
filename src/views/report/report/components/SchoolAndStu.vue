<template>
  <div ref="SchoolAndStu" class="w-full">
    ...
    <div v-if="provinceOrCity" class="w-full">
      <div class="w-full flex items-center justify-center">
        <PieChartReport :extend="extend" width="120mm" height="80mm" />
      </div>
      <p class="text-center m-b-10">{{ areaName }}各学段占比</p>
    </div>
    <div v-if="areaLevel == 3" class="w-full">
      ...
      <div class="w-full flex items-center justify-center">
        <p class="w-1/2 text-center m-b-10">{{ areaName }}各学段占比</p>
        <p class="w-1/2 text-center m-b-10">{{ areaName }}各学段人数占比</p>
      </div>
    </div>

    <p v-if="provinceOrCity" class="text-center m-b-3 font-bold">{{ areaName }}各县（市、区）学段数量</p>
    <n-table v-if="provinceOrCity && state.detail.prefixSchoolCountInfoVO" :single-line="false" class="text-center">
      <thead>
        <tr>
          <th>序号</th>
          <th>县(市、区)</th>
          <th v-if="state.detail.prefixSchoolCountInfoVO.kindergartenCount * 1">幼儿园</th>
          <th v-if="state.detail.prefixSchoolCountInfoVO.primarySchoolCount * 1">小学</th>
          <th v-if="state.detail.prefixSchoolCountInfoVO.middleSchoolCount * 1">初中</th>
          <th v-if="state.detail.prefixSchoolCountInfoVO.highSchoolCount * 1">
            <div>高中</div>
            <div>(普高/职高)</div>
          </th>
          <th>总数</th>
      ...
      <tbody>
        <tr v-for="(item, index) in state.detail.regionPrefixSchoolCounts" :key="item.region">
          <td>{{ index + 1 }}</td>
          <td>{{ item.region }}</td>
          <td v-if="state.detail.prefixSchoolCountInfoVO.kindergartenCount * 1">
            {{ item.prefixSchoolCountInfoVO.kindergartenCount }}
          </td>
          <td v-if="state.detail.prefixSchoolCountInfoVO.primarySchoolCount * 1">
            {{ item.prefixSchoolCountInfoVO.primarySchoolCount }}
          </td>
          <td v-if="state.detail.prefixSchoolCountInfoVO.middleSchoolCount * 1">
            {{ item.prefixSchoolCountInfoVO.middleSchoolCount }}
          </td>
          <td v-if="state.detail.prefixSchoolCountInfoVO.highSchoolCount * 1">
    ...
    <p v-if="areaLevel == 3" class="text-center m-b-3 font-bold">{{ areaName }}各学段情况</p>
    ...
    <div v-if="state.detail.prefixSchoolCountInfoVO && provinceOrCity" class="m-t-5 m-b-5 text-sm leading-6">
      {{ state.strDesc3 }}
    </div>
    <div v-if="provinceOrCity" class="w-full">
      <div class="w-full flex items-center justify-center">
        <PieChartReport :extend="extend2" width="120mm" height="80mm" />
      </div>
      <p class="text-center m-t-5 m-b-10">{{ areaName }}各学段人数占比</p>

      <p class="text-center m-b-3 font-bold">{{ areaName }}各县（市、区）学生数量</p>
      <n-table v-if="state.detail.prefixSchoolCountInfoVO" :single-line="false" class="text-center">
        <thead>
          <tr>
            <th>序号</th>
            <th>县(市、区)</th>
        ...
        <tbody>
          <tr v-for="(item, index) in state.detail.regionPrefixSchoolCounts" :key="item.region">
            ...
            <td>{{ item.region }}</td>
            <td v-if="state.detail.prefixSchoolCountInfoVO.kindergartenCount * 1">
              {{ item.prefixSchoolCountInfoVO.kindergartenStdCount }}
            </td>
            <td v-if="state.detail.prefixSchoolCountInfoVO.primarySchoolCount * 1">
              {{ item.prefixSchoolCountInfoVO.primarySchoolStdCount }}
            </td>
            <td v-if="state.detail.prefixSchoolCountInfoVO.middleSchoolCount * 1">
              {{ item.prefixSchoolCountInfoVO.middleSchoolStdCount }}
            </td>
            <td v-if="state.detail.prefixSchoolCountInfoVO.highSchoolCount * 1">
...
import { pie_per } from '@common/settings/chartSetting';
import { percentToFloat } from '../data.js';

import { getSchoolStdInfo, getSchoolStdInfo_r } from '@/api/report/index';
import { getSchoolStdInfoPro } from '@/api/report/index_province';
...
const provinceOrCity = computed(() => {
  return areaLevel == 1 || areaLevel == 2;
});
if (areaLevel == 2) {
  getSchoolStdInfo({
    planId: planId,
  ...
  getSchoolStdInfo_r({
    ...
    if (d?.prefixSchoolCountInfoVO?.schoolCount * 1) {
      state.gradeNum.push({
        gradeName: '合计',
        schoolNum: d.prefixSchoolCountInfoVO.schoolCount,
        manCount: d.prefixSchoolCountInfoVO.manStdCount,
        womenCount: d.prefixSchoolCountInfoVO.woManStdCount,
        count: d.prefixSchoolCountInfoVO.stdCount
      });
    }
  });
}
function setStr1() {
  if (state.detail.prefixSchoolCountInfoVO) {
    state.strDesc1 = `本次筛查涉及的${state.detail.prefixSchoolCountInfoVO.schoolCount}所学校中包括${
    ...
    state.strDesc1 = state.strDesc1.replace('包括、', '包括');
  }
}
function setStr2() {
  if (state.detail.prefixSchoolCountInfoVO) {
    state.strDesc2 = `其中，${state.detail.prefixSchoolCountInfoVO.kindergartenStdCount * 1 ? '幼儿园' : ''}${
    ...
    }${state.detail.prefixSchoolCountInfoVO.collegeStdCount * 1 ? '、大学' : ''}人数分别为${
      state.detail.prefixSchoolCountInfoVO.kindergartenStdCount * 1
        ? `${state.detail.prefixSchoolCountInfoVO.kindergartenStdCount * 1}人`
        : ''
    }${
      state.detail.prefixSchoolCountInfoVO.primarySchoolStdCount * 1
        ? `、${state.detail.prefixSchoolCountInfoVO.primarySchoolStdCount * 1}人`
        : ''
    }${
      state.detail.prefixSchoolCountInfoVO.middleSchoolStdCount * 1
        ? `、${state.detail.prefixSchoolCountInfoVO.middleSchoolStdCount * 1}人`
        : ''
    }${
      state.detail.prefixSchoolCountInfoVO.highSchoolStdCount * 1
        ? `、${state.detail.prefixSchoolCountInfoVO.highSchoolStdCount * 1}人`
        : ''
    }${
      state.detail.prefixSchoolCountInfoVO.collegeStdCount * 1
        ? `、${state.detail.prefixSchoolCountInfoVO.collegeStdCount * 1}人`
        : ''
    }
    。全区${state.detail.prefixSchoolCountInfoVO.kindergartenStdCount * 1 ? '幼儿园' : ''}${
      state.detail.prefixSchoolCountInfoVO.primarySchoolStdCount * 1 ? '、小学' : ''
    }${state.detail.prefixSchoolCountInfoVO.middleSchoolStdCount * 1 ? '、初中' : ''}${
      state.detail.prefixSchoolCountInfoVO.highSchoolStdCount * 1 ? '、高中' : ''
    }${state.detail.prefixSchoolCountInfoVO.collegeStdCount * 1 ? '、大学' : ''}学生比约为${
      state.detail.prefixPeopleRatio
    }， ${state.detail.maxPeoplePrefix}人数比例偏大，${state.detail.minPeoplePrefix}人数比例偏小。`;
    state.strDesc2 = state.strDesc2
      .replace('全区、', '全区')
      .replace('其中，、', '其中，')
      .replace('分别为、', '分别为');
  }
...
function setStr3() {
  state.strDesc3 = `其中，${state.detail.prefixSchoolCountInfoVO.kindergartenStdCount * 1 ? '幼儿园' : ''}${
    state.detail.prefixSchoolCountInfoVO.primarySchoolStdCount * 1 ? '、小学' : ''
  }${state.detail.prefixSchoolCountInfoVO.middleSchoolStdCount * 1 ? '、初中' : ''}${
    state.detail.prefixSchoolCountInfoVO.highSchoolStdCount * 1 ? '、高中' : ''
  }${state.detail.prefixSchoolCountInfoVO.collegeStdCount * 1 ? '、大学' : ''}人数分别为${
  ...
  state.strDesc3 = state.strDesc3.replace('其中，、', '其中，');
  state.strDesc3 = state.strDesc3.replace('分别为、', '分别为');
  state.strDesc3 = state.strDesc3.replace('全市、', '全市');
  state.strDesc3 = state.strDesc3.replace('全省、', '全省');
}
</script>
