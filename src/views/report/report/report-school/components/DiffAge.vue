<template>
  <div class="w-full">
    ...
    <p class="text-center m-b-10">{{ state.detail.name }}不同年龄近视率</p>
    <LineChartReport height="250px" width="95%" :extend="myExtend2" :label-map="labelMap" :chart-data="chartData" />
    <p class="text-center m-b-10">{{ state.detail.name }}不同年龄高度近视率</p>
    <p class="text-center m-t-5 m-b-3 font-bold">{{ state.detail.name }}不同年龄近视率</p>
        ...
        str =
          ...
          `<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background:${
            v.color.colorStops
              ...
              : v.color
          }"></span>${v.seriesName}：${(v.value.myopiaMyopiaRate * 100).toFixed(2) + '%'}<br>`;
      });
      return str;
    }
  },
