<template>
  <div class="w-full">
    <div id="MyopiaState" class="text-base font-bold m-b-1">近视情况</div>
    ...
    <div v-if="state.detail.myopiaStdCount" class="w-full">
      ...
      <div class="flex-1 flex flex-wrap justify-center m-b-5">
        <div class="flex w-50% p-t-24px">
          <p class="w-40%">近视人数（男/女）</p>
          <p class="w-60%">
    ...
    <div v-if="areaLevel != 6" class="w-full">
      {{ state.schoolStr }}
      ...
      <p class="text-center m-b-3 font-bold">
        {{ areaName }}各{{ areaLevel == 4 ? '年级' : '班级' }}年级男、女生近视情况
      </p>
      <n-table :single-line="false" class="text-center">
        ...
        <tbody v-if="areaLevel == 5">
          <tr v-for="(item, index) in state.detail.classMyopiaInfoVOS" :key="item.className">
            <td>{{ index + 1 }}</td>
      ...
      <p class="text-center m-b-10">{{ areaName }}各{{ areaLevel == 4 ? '年级' : '班级' }}近视程度分布情况</p>
    </div>
    <p v-if="areaLevel != 6" class="text-center m-b-3 font-bold">
      {{ areaName }}各{{ areaLevel == 4 ? '年级' : '班级' }}近视程度分布情况
    </p>
    <n-table v-if="areaLevel != 6" :single-line="false" class="text-center">
      ...
      <tbody v-if="areaLevel == 4 || areaLevel == 5">
    ...
    <div v-if="areaLevel == 4" class="w-full flex m-t-10 items-center justify-between">
      <div class="flex flex-col items-center w-1/2">
        ...
        />
        <div>{{ areaName }}各{{ areaLevel == 4 ? '年级' : '班级' }}近视率变化趋势</div>
      </div>
      <div class="flex flex-col items-center w-1/2">
        <LineChartReport
          height="250px"
        ...
        />
        <div>{{ areaName }}各{{ areaLevel == 4 ? '年级' : '班级' }}高度近视率变化趋势</div>
      </div>
    </div>
    <n-table v-if="areaLevel == 6" :single-line="false" class="text-center">
      <thead>
      ...
      <tbody v-if="state.detail">
        <tr v-for="(item, index) in state.detail.studentMyopiaInfos" :key="item.studentName">
          <td>{{ index + 1 }}</td>
          <td>{{ item.studentName }}</td>
          <td :style="{ background: item.isLowMyopia ? '#caf982' : '' }">{{ item.isLowMyopia ? '√' : '×' }}</td>
          <td :style="{ background: item.isMediumMyopia ? '#f59a23' : '' }">{{ item.isMediumMyopia ? '√' : '×' }}</td>
          <td :style="{ background: item.isHighMyopia ? '#ec808d' : '' }">{{ item.isHighMyopia ? '√' : '×' }}</td>
        </tr>
      </tbody>
    </n-table>
    <div class="m-b-5 text-sm leading-6">注：非低度、中度、高度近视学生不参与统计</div>
    <report-divider />
...
import { getOverallMyopiaInfo_s, getOverallMyopiaInfo_g, getOverallMyopiaInfo_c } from '@/api/report/index';
...
let { planId, schoolId, schoolType, areaName, areaLevel, gradeId, classId } = userStore.getAreaData;
let state = reactive({
  m_degree: [],
  schoolStr: '',
  detail: {}
...
const extend = reactive({
  ...
  color: ['#00CE74', '#FCC40D', '#FF5B3E', '#3E9EFF', '#CEE6FF'],
  series: [
    {
      name: '近视比例',
      type: 'pie',
...
let labelMap_s2 = reactive({
  myopiaStdCount: '低度近视',
  manMyopiaStdCount: '中度近视',
  womanMyopiaStdCount: '高度近视'
});
...
if (areaLevel == 4) {
  getOverallMyopiaInfo_s({
    ...
    chartData_s.rows = JSON.parse(JSON.stringify(d.gradeMyopiaInfos)).filter(v => v.gradeName != '合计');
    chartData_s2.rows = JSON.parse(JSON.stringify(d.gradeMyopiaInfos)).filter(v => v.gradeName != '合计');
    let arr2 = [];
    d.gradeMyopiaInfos.forEach(v => {
      let obj = {
        name: '',
  ...
  getOverallMyopiaInfo_g({
    ...
    let str = '';
    let arr = [];
    d.classMyopiaInfoVOS.forEach(v => {
      if (v.className != '合计') {
    ...
    str += '。';
    str +=
      d.maxClass.className +
      '近视率最高为：' +
      floatToPercent(d.maxClass.myopiaRate) +
      '；' +
      d.minClass.className +
      '近视率最低为：' +
      floatToPercent(d.minClass.myopiaRate) +
      '。';
    state.schoolStr = str.replace('分别为：，', '分别为：');
    chartData2.rows = JSON.parse(JSON.stringify(d.classMyopiaInfoVOS));
    chartData3.rows = JSON.parse(JSON.stringify(d.classMyopiaInfoVOS));

    chartData_s.columns = ['className', 'myopiaStdCount', 'manMyopiaStdCount', 'womanMyopiaStdCount'];
    chartData_s.rows = JSON.parse(JSON.stringify(d.classMyopiaInfoVOS)).filter(v => v.className != '合计');

    chartData_s2.columns = ['className', 'myopiaStdCount', 'manMyopiaStdCount', 'womanMyopiaStdCount'];
    chartData_s2.rows = JSON.parse(JSON.stringify(d.classMyopiaInfoVOS)).filter(v => v.className != '合计');
    let arr2 = [];
    d.classMyopiaInfoVOS.forEach(v => {
      let obj = {
        name: '',
  ...
  getOverallMyopiaInfo_c({
    ...
    chartData_s.columns = ['className', 'myopiaStdCount', 'manMyopiaStdCount', 'womanMyopiaStdCount'];
    chartData_s.rows = JSON.parse(JSON.stringify(d.classMyopiaInfoVOS));

    chartData_s2.columns = ['className', 'myopiaStdCount', 'manMyopiaStdCount', 'womanMyopiaStdCount'];
    chartData_s2.rows = JSON.parse(JSON.stringify(d.classMyopiaInfoVOS));
  });
}
</script>
