...
<template>
  <div class="text-8">
    <n-spin :show="loading">
        ...
        <n-form ref="taskRef" :model="taskForm" :rules="taskRules">
          <n-card v-if="!isSelfCheck" title="计划信息" class="shadow-sm mb-3">
            <n-grid x-gap="30" :cols="3">
              <n-gi>
                <n-form-item label="筛查学校" path="schoolId">
                  <!-- <n-select v-model:value="planInfo.orgName" /> -->
                  <n-select
                    v-model:value="taskForm.schoolId"
                    :disabled="isEdit"
              ...
              <n-gi v-if="isSelfCheck">
                <n-form-item label="筛查学校" path="schoolId">
                  <n-select
                    ...
                    label-field="schoolName"
                    value-field="schoolId"
                    @update:value="handleSchoolChanges"
                  />
                </n-form-item>
              </n-gi>
              <n-gi v-if="isSelfCheck">
      ...
      <n-space :justify="currentRef === 3 ? 'center' : 'end'" class="bg-light mb-20 p-5 shadow-sm">
        <template v-if="currentRef === 1">
...
import { getAssociateAllSchool } from '@common/api/examine/plan';
...
const taskData = {
  schoolId: query.schoolId ? query.schoolId : null,
  planId: null,
  taskName: null,
  dateValue: null,
  equipmentIds: null,
...
const init = async () => {
  loading.value = true;
  let apiList = [getDeviceList({ orgId }), getTaskPersonList({ orgId, schoolId: taskForm.schoolId })];
  if (isSelfCheck.value) {
    apiList.push(getApplicationList({ orgId, appType }));
    apiList.push(getClassListFn());
  } else {
    apiList.push(getTaskPlanList({ schoolId: taskForm.schoolId, appType, isAll: isEdit.value ? 1 : 0 })); // 编辑获取全部计划，新增需过滤自动模式和未开始的计划
  }
  try {
...
const getClassListFn = () => {
  return new Promise(resolve => {
    const { planId, schoolId } = taskForm;
    // let params = { orgId };
    let params = { orgId: schoolId };
...
const getSchoolNameList = async () => {
  schoolNameList.value = await getAssociateAllSchool();
};

// 筛查学校、所属计划级联
const handleSchoolChange = async (a, option) => {
  taskForm.planId = null;
  // 更新下拉列表
  taskPlanList.value = await getTaskPlanList({ schoolId: taskForm.schoolId, appType });
  if (!isSelfCheck.value) {
    taskForm.taskName = option.schoolName;
    console.log(a, ' taskForm.taskschoolNameName', option);
  }

  init();
};

const handleSchoolChanges = async () => {
  init();
};

const handlePlanChange = async (val, option) => {
  Object.assign(planInfo, option);

  console.log(taskForm.dateValue, 'taskForm.dateValue ', option);
  const { startTime, endTime } = option;
  // TODO 设置开始日期-结束日期
  taskForm.dateValue = [startTime.substr(0, 10), endTime.substr(0, 10)];
...
const getScreenOptionsFn = async () => {
  ...
  try {
    ...
    const { appId, appVersion: version } = appGroupInfo;
    const api = isSelfCheck.value ? getScreenOptions : getPlanScreenOptions;
    let params = isSelfCheck.value ? { appId, version } : { planId, schoolId };
...
onBeforeMount(() => {
  if (taskForm.schoolId || isEdit.value) init();
  getSchoolNameList();
});
...
const submit = async () => {
  ...
  try {
    ...
    screenOptionList.value?.map(({ itemList }) => {
      itemList?.map(({ id, planId, taskId, disabled: _d, ...info }) => {
        let opt = isEdit.value ? { id, planId, taskId, ...info } : { ...info };
        screeningOpitons.push(opt);
      });
      // itemList?.map(({ screeningTypeId, screeningItemId, screeningItemName, selection, screeningItemCode }) => {
      //   screeningOpitons.push({ screeningTypeId, screeningItemId, screeningItemName, selection, screeningItemCode });
      // });
    });
    let params = {
      orgId,
      schoolId: taskForm.schoolId,
      ...info,
      startTime,
      endTime,
